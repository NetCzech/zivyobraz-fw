...
// GRAYSCALE (2bpp, 4 pixels/byte, MSB first): pixel x → bits (7-6 - (x%4)*2)
void fillPixelRunGrayscale(uint8_t *buffer, uint16_t startX, uint16_t count, uint8_t grey)
{
  if (count == 0)
    return;

  const uint8_t value = grey >> 6u; // 0-3
  ...
  if (startByte == endByte)
  {
    // All in one byte – write pixel-by-pixel
    for (uint16_t x = startX; x < endX; x++)
      packPixel4G(buffer, x, grey);
    return;
  }
...
void fillPixelRun4C(uint8_t *buffer, uint16_t startX, uint16_t count, uint8_t color4)
{
  ...
  const uint8_t pattern = (uint8_t)((color4 << 6u) | (color4 << 4u) | (color4 << 2u) | color4);
  const uint16_t endX = startX + count;
